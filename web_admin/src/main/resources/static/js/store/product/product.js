$(function () {
    layui.use(['index', "table", "form"], function () {
       var table = layui.table;
        ...
        table.render({
            elem: '#dataList'
            , url: ctx + '/product/tabData?type='+type
            , cols: [[
                {type: 'checkbox', fixed: 'left',width:'5%'}
                , {field: 'name', title: '商品名称',width:'20%'}
                , {field: 'shopName', title: '归属店铺',width:'15%'}
                , {field: 'catName', title: '类别', align: 'center',width:'10%'}
                , {field: 'specs', title: '使用说明', align: 'left',width:'20%'}
                , {field: 'currentPrice', title: '价格',width:'7%'}
        ...
        form.on('switch(state)', function (data) {
            ...
            $.post(ctx + "/product/updateState", {id: id, state: val}, function (res) {
                if (res.errorCode != -1) {
        ...
        table.on('tool(dataList)', function (obj) {
            ...
            if (obj.event === 'del') {
                pLayer.confirm('确定删除此商品？', function (index) {
                    pLayer.close(index);
                    //后台删除
                    $.post(ctx + "/product/del", {id: data.id}, function (res) {
                        if (res.errorCode != -1) {
                            layer.msg(res.errorMsg, {icon: 6});
                            table.reload("dataList");
                        } else {
                ...
                pLayer.open({
                    type: 2
                    , title: '编辑商品'
                    , content: ctx + "/product/formPage?id=" + data.id+"&type="+type
                    , maxmin: true
                    , area: calScreenSize()
                    , btn: ['确定', '取消']
        ...
        var active = {
            batchdel: function () {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    ...
                    //后台批量删除
                    $.post(ctx + "/product/delBatch",{ids:JSON.stringify(ids)}, function (res) {
                        if (res.errorCode != -1) {
                            layer.msg(res.errorMsg, {icon: 6});
                            table.reload("dataList");
                        } else {
            ...
            add: function () {
                pLayer.open({
                    type: 2
                    , title: '添加商品'
                    , content: ctx + "/product/formPage?type="+type
                    , maxmin: true
                    , area: calScreenSize()
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
