$(function () {
    layui.use(['index', "table", "form"], function () {
       var table = layui.table;
        ...
        table.render({
            elem: '#dataList'
            , url: ctx + '/driverProdRel/tabData?unShopId='+shopId+"&type="+type
            , cols: [[
                {type: 'checkbox', fixed: 'left',width:'5%'}
                , {field: 'name', title: '名称',width:'20%'}
                , {field: 'catName', title: '类别', align: 'center',width:'15%'}
                , {field: 'currentPrice', title: '价格',width:'10%'}
                , {field: 'classes', title: '班次', align: 'center',width:'15%'}
                , {field: 'motorcycle', title: '车型', align: 'center',width:'10%'}
        ...
        table.render({
            elem: '#selectDataList'
            , url: ctx + '/driverProdRel/selectTabData?shopId='+shopId+"&type="+type
            , cols: [[
                {type: 'checkbox', fixed: 'left',width:'5%'}
                , {field: 'name', title: '名称',width:'20%'}
                , {field: 'catName', title: '类别', align: 'center',width:'15%'}
        ...
        table.on('tool(dataList)', function (obj) {
            ...
            if (obj.event === 'addRel') {
                pLayer.confirm('确定添加吗？', function (index) {
                    pLayer.close(index);
                    //后台删除
                    $.post(ctx + "/driverProdRel/addRel", {shopId:shopId,productId: data.id}, function (res) {
                        if (res.errorCode != -1) {
        ...
        table.on('tool(selectDataList)', function (obj) {
            ...
            if (obj.event === 'delRel') {
                pLayer.confirm('确定删除吗？', function (index) {
                    pLayer.close(index);
                    //后台删除
                    $.post(ctx + "/driverProdRel/delRel", {shopId:shopId,productId: data.id}, function (res) {
                        if (res.errorCode != -1) {
                            layer.msg(res.errorMsg, {icon: 6});
                            table.reload("dataList");
                            table.reload("selectDataList");
