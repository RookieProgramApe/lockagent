$(function () {
    layui.use(['index',"table","form"],function () {
        ...
        table.render({
            elem: '#dataList'
            ,url: ctx+ '/productCat/categoryTabData?type='+type //模拟接口
            ,cols: [[
                {type: 'numbers'}
                ,{field: 'imageUrl', title: '图标',templet:'#imageUrlTpl'}
        ...
        form.on('switch(stateFilter)', function(data){
            ...
            $.post(ctx+"/productCat/updateState",{id:id,state:val},function (res) {
                if(res.errorCode != -1){
        ...
        table.on('tool(dataList)', function(obj){
            ...
            if(obj.event === 'del'){
                pLayer.confirm('确定删除此分类？', function(index){
                    pLayer.close(index);
                    //后台删除
                    $.post(ctx+"/productCat/del",{id:data.id},function (res) {
                        if(res.errorCode != -1){
                            layer.msg(res.errorMsg, {icon: 6});
                ...
                pLayer.open({
                    type: 2
                    ,title: '编辑分类'
                    ,content: ctx+"/productCat/formPage?id="+ data.id
                    ,maxmin: true
                    ,area: calScreenSize()
                    ,btn: ['确定', '取消']
        ...
        var active = {
            add: function(){
                pLayer.open({
                    type: 2
                    ,title: '添加分类'
                    ,content: ctx+"/productCat/formPage?type="+type
                    ,maxmin: true
                    ,area: calScreenSize()
                    ,btn: ['确定', '取消']
                    ,yes: function(index, layero){
