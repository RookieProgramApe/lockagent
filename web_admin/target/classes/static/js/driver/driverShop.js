$(function () {
    layui.use(['index',"table", "form"], function () {
       var table = layui.table;
        ...
        table.render({
            elem: '#dataList'
            , url: ctx + '/driverSchool/tabData?type='+type
            , cols: [[
                ...
                , {field: 'state', title: '状态', templet: '#buttonTpl', align: 'center',width:'8%'}
        ...
        form.on('switch(isHot)', function (data) {
            ...
            $.post(ctx + "/driverSchool/updateShopHot", {id: id, isHot: val}, function (res) {
                if (res.errorCode != -1) {
        ...
        table.on('tool(dataList)', function (obj) {
            ...
            if (obj.event === 'loadProdRel') {
                ...
                pLayer.open({
                    type: 2
                    , title: '关联商品或套餐'
                    , content: ctx + "/driverProdRel/list?shopId=" + data.id+"&type="+type
                    , maxmin: true
                    , area: calScreenSize()
                    , btn: ['确定', '取消']
                ...
                pLayer.confirm('确定删除吗？', function (index) {
                    pLayer.close(index);
                    //后台删除
                    $.post(ctx + "/driverSchool/del", {id: data.id}, function (res) {
                        if (res.errorCode != -1) {
                            layer.msg(res.errorMsg, {icon: 6});
                            table.reload("dataList");
                        } else {
                ...
                pLayer.open({
                    type: 2
                    , title: '编辑'
                    , content: ctx + "/driverSchool/formPage?id=" + data.id
                    , maxmin: true
                    , area: calScreenSize()
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
        ...
        var active = {
            batchdel: function () {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    ...
                    //后台批量删除
                    $.post(ctx + "/driverSchool/delBatch",{ids:JSON.stringify(ids)}, function (res) {
                        if (res.errorCode != -1) {
                            layer.msg(res.errorMsg, {icon: 6});
                            table.reload("dataList");
                        } else {
            ...
            add: function () {
                pLayer.open({
                    type: 2
                    , title: '添加'
                    , content: ctx + "/driverSchool/formPage?type="+type
                    , maxmin: true
                    , area: calScreenSize()
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
